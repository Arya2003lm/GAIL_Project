...
GOAL_Y_OFFSET = -3.0  # 参考轨迹终点 y 偏移：y -> y - 4
...
def plot_reference_trajs(
    ...
    if show_expert:
        ...
        ax.legend(handles=[legend_handles[1]], fontsize=16)

    ax.set_xlabel("主车横向位置 X（米）", fontsize=20)
    ax.set_ylabel("主车纵向位置 Y（米）", fontsize=20)
    ax.set_title(
        f"参考轨迹可视化（共 {len(ep_indices)} 条）\n"
        f"直线段：起点 → (-2, y_start)  |  圆弧段：转弯点 → 目标点",
        fontsize=20,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="生成并可视化参考轨迹")
    parser.add_argument("--ep", type=int, nargs="+", default=None,
                        help="要显示的轨迹索引（默认：全部）")
    parser.add_argument("--save", action="store_true",
                        help="保存图片到 evaluate/ref_trajs04.svg")
    parser.add_argument("--no-expert", action="store_true",
                        help="不显示真实轨迹，只显示参考轨迹")
    parser.add_argument("--data-dir", default=DATA_DIR)
    args = parser.parse_args()

    print("正在加载数据...")
    trajs, dfs = load_all_with_dataframes(args.data_dir)
    print(f"已加载 {len(dfs)} 条轨迹")

    save_path = "./evaluate/ref_trajs04.svg" if args.save else None
