...
# 若希望固定加载某个 BC 模型，可填写模型名或绝对路径。
# 例如: "bc_policy_hv_ax_ay_003" 或 "E:/.../checkpoints/bc_policy_hv_ax_ay_003.zip"
# 为 None 时，沿用 BC_POLICY_NAME 的“最新版本自动解析”逻辑。
...
# -------- 三组可切换实验配置 --------
# 可选: "ppo" | "airl" | "long"
EXPERIMENT_PROFILE = "long"

GLOBAL_SEED = 42
N_ENVS = 4

# 先给一组默认值，随后由 PROFILE_CONFIGS 覆盖
...
ENV_LOG_INTERVAL_STEPS = 2000  # 每约 N 步打印一次环境奖励分量
...
class EnvRewardLogVecEnvWrapper(VecEnvWrapper):
    """仅用于日志：每隔 log_interval_steps 打印一次环境奖励分量均值。"""

    def __init__(self, venv, log_interval_steps: int = 2000):
        super().__init__(venv)
    ...
    def step_wait(self):
        ...
        for info in infos:
            self._acc["env_reward"]      += float(info.get("original_env_rew", 0.0))
        ...
        if self.log_interval_steps > 0 and self._step_count >= self.log_interval_steps and self._samples > 0:
...
def train_airl(
	...
	if ENV_LOG_INTERVAL_STEPS > 0:
		trainer.venv_train = EnvRewardLogVecEnvWrapper(
			trainer.venv_train,
...
if __name__ == "__main__":
	...
	if RUN_AIRL_SMOKE_TEST:
		...
		print("\n[AIRL Smoke] 在验证集上评估：")
		evaluate_action_error(airl_smoke_trainer, val_trajs)
		evaluate_position_ade_fde(airl_smoke_trainer.policy, val_dfs, title="AIRL Smoke 位置 ADE/FDE")
	...
	if RUN_AIRL_FULL_TRAIN:
		...
		print("\n[AIRL Full] 在验证集上评估：")
		evaluate_action_error(airl_full_trainer, val_trajs)
		evaluate_position_ade_fde(airl_full_trainer.policy, val_dfs, title="AIRL Full 位置 ADE/FDE")
